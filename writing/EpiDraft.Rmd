...
nodes_midatlantic <- html_2015 %>% map(~html_nodes(.x, 'tr:nth-child(13) td:nth-child(12)'))
madat <- nodes_midatlantic %>% map(~html_text(.x))
as.numeric(madat)

nodes_nyc <- html_2015 %>% map(~html_nodes(.x, 'tr:nth-child(16) td:nth-child(12)'))
nycdat <- nodes_nyc %>% map(~html_text(.x))
as.numeric(nycdat)

nodes_pa <- html_2015 %>% map(~html_nodes(.x, 'tr:nth-child(17) td:nth-child(12)'))
padat <- nodes_pa %>% map(~html_text(.x))
as.numeric(padat)
...
# Putting two years together

leg_cases <- bind_rows(leg_cases_2015, leg_cases_2016, leg_cases_2017)
...
#For the earlier years we only have cumulative data.  Could take differences via code? 2006 is the first year with variable for current week. NYC data not available many years, so I'll use Mid-Atlantic region.
