...
leg_cases[,c(1,4,8,9, 10)] %>% group_by(moyr) %>% summarise(sum=sum(penn, na.rm=T)) %>% ggplot(aes(x=moyr, y=sum))+ geom_bar(stat="identity") + labs(x="Date", y="Cases", title = "Monthly Legionnaire's Cases for Pennsylvania")
...
simdat <- eesim(n=3650, n_reps=1, rr=1.1, exposure_type = "binary", central = c(0.048, 0.035, 0.046, 0.036, 0.035, 0.055, 0.053, 0.067, 0.070, 0.053, 0.033, 0.067), exposure_trend = "monthly", outcome_trend="cos1", average_outcome = 5.3/7, custom_model = spline_mod)

#Create new month/year variable to be able to aggregate by months
simdat$simulated_datasets[[1]]$moyr <- ymd(strtrim(simdat$simulated_datasets[[1]]$date, width=7), truncated=3)

#Plot of monthly cases 
simdat$simulated_datasets[[1]] %>% group_by(moyr) %>% summarise(sum=sum(outcome, na.rm=T)) %>% ggplot(aes(x=moyr, y=sum))+ geom_bar(stat="identity") + labs(x="Date", y="Cases", title = "Simulated Monthly Legionnaire's Cases")
```
