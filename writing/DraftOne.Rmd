...
power_calc <- function(varying, values, n_reps, n=NULL, central, sd, exposure_type,
                       ...
                       model, df_year=7){
  if(varying == "n"){
    rep_df <- n %>% map(create_sims, n_reps=n_reps, central=central, sd=sd, 
                        exposure_type = exposure_type,
      ...
      fits <- rep_df %>% map(fit_mods, model=model)
      power <- fits %>% map(power_beta) #makes a list, want to extract the values of power and put in a data frame with values of n.
    }
  else if(varying == "rr"){
    rep_df <- rr %>% map(create_sims, n=n, n_reps=n_reps, central=central, sd=sd, 
                        exposure_type = exposure_type,
                        exposure_trend=exposure_trend,exposure_amp=exposure_amp,
                        average_outcome=average_outcome,outcome_trend=outcome_trend,
                        outcome_amp = outcome_amp, start.date = start.date)
      fits <- rep_df %>% map(fit_mods, model=model)
      power <- fits %>% map(power_beta) #makes a list, want to extract the values of power and put in a data frame with values of rr.
    }
  if(plot == TRUE){
...
power_calc(varying = "n", values = c(5, 10, 25), n_reps = 3, central = 100, sd=10, exposure_type = "continuous", exposure_trend = "cos1", exposure_amp = .6, average_outcome=22, outcome_trend = "no trend", outcome_amp = .6, rr=1.1, start.date = "2000-01-01", model = "spline", df_year = 1)
```
...
rep_df <- n %>% map(create_sims, n_reps=3, central=100, sd=10, 
                        exposure_type = "continuous",
                        exposure_trend= "cos1",exposure_amp=.6,
                        average_outcome=22,outcome_trend="no trend",
                        outcome_amp = .6, rr=1.1, start.date = "2000-01-01")
rep_df
#This works, but why are the outcomes crazy? Oh noooooooo :( 
```
