...
First, we need to generate the explanatory exposure variable.  Here is a function for binary exposure in which the probability varies by season.  Later we will add a function for continuous exposure which varies by season (~Normal(mu, sigma)). 
...
Now a function to calculate the trend variable. Choices for trend are cvd, cos1, cos2, cos3, linear, curvilinear, cos1linear.

```{r}
calc_t <- function(n, trend){
  day <- c(1:n)
  if (trend == "cos1"){
...
sim_outcome <- function(n, mean_out, t, exposure, rr){
  day <- c(1:n)
  rr <- ifelse(exposure == 1, rr, 1)
  mean_outcome <- mean_out*t*rr
  sim_outcome <- rpois(n, mean_outcome)
  return(sim_outcome)
}
```
...
```{r}
#Make this function output a dataframe with date, x, etc.
sim_seasdata <- function(n, rr, trend, x_type="binary", p, mean_out){
  if(x_type == "binary"){
    x <- season_binexp(n, p)
  } 
  ...
  t <- calc_t(n, trend)
  outcomes <- sim_outcome(n, mean_out, t, x, rr)
  return(outcomes)
}
```
