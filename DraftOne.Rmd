...
```{r}
sim_data <- function(n, rr, x_type = "binary", trend = "constant",
                     start.date = "2000-01-01", ...){
        
        require(dplyr)
        t <- calc_t(n, trend)
        if(x_type == "binary" & trend = "constant"){
                x <- binary_exposure(n = n, ...)
                simdata <- data.frame(x, sim_binout(n=n, ...))
        } else if(x_type == "continuous" & trend=="constant"){
                x <- continuous_exposure(n = n, ...)
                simdata <- data.frame(x, sim_contout(n=n, ...))
        } else if(x_type == "binary" & trend != "constant"){
                x <- season_binexp(n = n, ...)
                simdata <- data.frame(x, sim_binout(n=n, ...))
        } else if(x_type == "continuous" & trend != "constant"){
                x <- season_contexp(n = n, ...)
                simdata <- data.frame(x, sim_contout(n=n, ...))
        }
