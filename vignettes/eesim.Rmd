...
eesim_overall_output <- data_frame(element = paste0("`",
                                                    colnames(ex_sim[[3]]),
                                                    "`"),
                                   desc = c("**Mean beta hat**: The mean of the estimated log relative rate over all simulations.",
                                            ...
                                            "**95% confidence inverval coverage**: Percent of simulations for which the 95% confidence interval estimate includes the true value. [Johnson2015, Beaujean2014]",
                                            "**Power**: Percent of simulations in which a true effect, with an effect size given by an alternative hypothesis, is identified with a p-value of 0.05 or lower; i.e., probability of rejecting the null hypothesis when the the specified alternative hypothesis is true. [Bellan2015, Johnson2015, Gaye2015, Green2016, Beaujean2014]"))
...
We can also customize the randomization of the exposure values from the trend line using the cust_expdraws argument.  The default randomization for continuous exposure values is from the normal distribution where the mean is the trend line and the standard deviation is specified by the 'sd' argument in the 'eesim' function.  The default for binary exposure values is a binomial distribution where the probability is the trend line. 

A function for custom randomization of exposure values must input values "n" and "mean" for continuous exposure and "n" and "prob" for binary exposure. Any arguments other than these should be put in a list for the 'cust_expdraw_args' argument. The function must output a vector of numeric exposure values. 

Here is an example of using a custom function for the randomization of draws off of the trend line:

```{r,fig.width = 5, fig.height = 3, fig.align = "center"}
newrgamma <- function(n, mean, rate){
  out <- rgamma(n=n, shape=mean, rate=rate)
  return(out)
}
customexp2 <- sim_exposure(n=1000, exposure_type = "continuous", cust_exp_func = sintrend, cust_exp_args = list(n=1000, mu = 75, y=3), cust_expdraw = newrgamma, cust_expdraw_args = list(rate=2))
qplot(customexp2$date, customexp2$x)+geom_point()+labs(title = "Exposure values with a custom trend and custom randomization", x="Date", y="Exposure")+theme_bw()
```

These custom exposure functions can be called with eesim:
...
ex_sim2 <- eesim(n_reps = 3, n = 10, 
                ...
                cust_expdraw = newrgamma,
                cust_expdraw_args = list(rate=2),
                average_outcome = 22, rr = 1.01, 
                custom_model = spline_mod, custom_model_args = list(df_year = 1))
ex_sim2
```

[For this step, is there a way to customize how random values are drawn from the distribution once you've figured out the expected value? For the data from Chicago that I've added to the start of the vignette, it looks like ozone concentration is really closer to normally distributed if you do a square root transformation first, then draw from a normal with the expected value of the square root of ozone concentration, then square to get back to the right scale for using to simulate the outcome. As another example, it could be useful to be able to simulate daily values under the case of autocorrelation. I think both of these would come in at the level of simulating from the distribution.]
...
[Using this package to re-do the analysis in Bateson and Schwartz, 1999]

```{r eval = FALSE}
case_study_1 <- eesim(n_reps = 10, n = 1096, 
           central = 0.25,
           exposure_type = "binary", exposure_trend = "cos1",
           exposure_amp = -0.2, average_outcome = 22,
           outcome_trend = "cos1linear", outcome_amp = 0.2, 
           rr = 1.10, model = "casecrossover")
case_study_1[[2]]
coverage_plot(sample_n(case_study_1[[1]], 100), true_param = 1.10)
```
