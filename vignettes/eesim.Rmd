...
head(testbin)
qplot(testbin$date, testbin$x)+geom_point()+labs(title = "Binary exposure data with montly trend")
```
...
qplot(testout$date, testout$outcome)+geom_point()+labs(title = "Health outcomes with a linear trend") + theme_bw()
```
...
head(customexp)
qplot(customexp$date, customexp$x)+geom_point()+labs(title = "Exposure values with a custom trend")
```
...
The outcome baseline is the values the user expects the outcomes to take on without exposure factored in. The user may write a function to specify the trend of the baseline, then use it as an input in sim_outcome or eesim. Here is an example of creating a custom baseline:
